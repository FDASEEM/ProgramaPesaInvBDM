...
class ModernPesaUSBMonitor:
    def __init__(self, root):
        ...
        # No cargar mapeo automáticamente al inicio.
        # El usuario puede cargarlo manualmente con el botón "📥 Cargar RUTs".
#
    ...
    def create_history_card(self, parent):
        ...
        card = tk.Frame(parent, bg='#ffffff', relief='solid', bd=1)
        card.grid(row=2, column=0, sticky='nsew', pady=(0, 15), padx=(5, 10))
    ...
    def create_saved_measurements_card(self, parent):
        ...
        card = tk.Frame(parent, bg='#ffffff', relief='solid', bd=1)
        card.grid(row=2, column=1, sticky='nsew', pady=(0, 15), padx=(10, 5))
        ...
        tk.Label(header, text="💾 Mediciones Guardadas", bg='#e67e22', fg='white',
                font=('Segoe UI', 12, 'bold')).pack(side='left', padx=15, pady=10)
        ...
        columns = ("Timestamp", "RUT", "Nombre", "Peso")
        ...
        self.tree.heading("Peso", text="⚖️ Peso (kg)")
        ...
        if filename:
            try:
                ...
                for item in self.tree.get_children():
                    ...
                    # valores: [timestamp, rut, name, peso_str]
                    datos.append({
                        "timestamp": valores[0],
                        "rut": valores[1],
                        "name": valores[2],
                        "peso_kg": float(valores[3])
                    })
                
                with open(filename, 'w', encoding='utf-8') as f:
                    json.dump(datos, f, indent=2, ensure_ascii=False)
                
                self.show_success_notification(f"Datos exportados exitosamente")
